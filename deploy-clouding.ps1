...
$sshCmd = 'pm2 flush 2>/dev/null; journalctl --vacuum-size=10M 2>/dev/null; npm cache clean --force 2>/dev/null; rm -rf /tmp/* /var/tmp/* ~/.cache/* /root/.cache/* ~/.pm2/logs/* /root/.pm2/logs/* /var/log/*.gz ~/islamicreelsupdated/tmp ~/islamicreelsupdated/.output/tmp 2>/dev/null; DIR=$(find / -maxdepth 5 -name package.json 2>/dev/null | grep -v node_modules | grep -v "\.output" | head -n 1 | xargs dirname); echo "Found root project directory: $DIR"; cd "$DIR" && git pull origin main && npm install --force && npm run build && (systemctl restart nginx 2>/dev/null || true) && pm2 delete all 2>/dev/null; if systemctl is-active --quiet nginx; then echo "Nginx active -> Starting app on PORT=3000"; PORT=3000 pm2 start .output/server/index.mjs --name islamic-reels; else echo "No Nginx -> Starting app directly on PORT=80"; PORT=80 pm2 start .output/server/index.mjs --name islamic-reels; fi && pm2 save && pm2 status && df -h'
