...
const HIGHLIGHT_KEYWORDS =
  /^(Аллах|Коран|Корана|Пророк|Пророкът|Хадис|Сура|Аят|Рай|Дженнет|Дженнета|Дуа|Иман|Благословение|Милост|Търпение|Надежда|Успех|Мухаммад|Господ|Господар|Победа|Спокойствие|Защита|Сърце|Сърцето|Живот|Време|Времето|Истина|Истината|Светлина|Зло|Добро|Вяра|Вярата)[.,!?…]?$/i;
...
async function loadImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setTimeout(
      () => reject(new Error("Фоновото изображение се зарежда твърде бавно")),
      10_000,
    );
    ...
    img.onerror = () => {
      window.clearTimeout(timer);
      reject(new Error("Не успях да заредя фоновото изображение"));
    };
    img.src = src;
  });
}
...
export async function renderVideo(opts: VideoOptions): Promise<{ blob: Blob; mimeType: string }> {
  if (typeof MediaRecorder === "undefined") {
    throw new Error(
      "Този браузър не поддържа директно видео рендиране. Избери 'Сървърно рендиране' от падащото меню за 100% стабилност.",
    );
  }
  ...
  try {
    ...
    const bg =
      !bgVideo && opts.backgroundUrl ? await loadImage(opts.backgroundUrl).catch(() => null) : null;
    ...
    if (opts.audioUrl) {
      try {
        ...
        const arr = await withTimeout(
          res.arrayBuffer(),
          15_000,
          "Аудио файлът не се изтегли докрай",
        );
        ...
        const originalBuf = await withTimeout(
          audioCtx.decodeAudioData(arr.slice(0)),
          15_000,
          "Аудиото не можа да се декодира",
        );
        ...
        audioSource.onended = () => {
          ...
          if (audioEndedAtWall === null) {
            audioEndedAtWall = renderStartedAt
              ? (performance.now() - renderStartedAt) / 1000
              : duration;
          }
        };
        ...
        console.log(
          "[render-video] audio decoded:",
          originalBuf.duration.toFixed(2),
          "s, padded to:",
          paddedBuf.duration.toFixed(2),
          "s",
        );
      } catch (err) {
        ...
        if (opts.requireAudio) {
          throw new Error(
            "Аудиото не можа да се зареди за видеото. Опитай пак с бутона 'Чуй гласа' и после рендирай.",
          );
        }
        audioCtx = null;
        audioDest = null;
    ...
    canvasVideoTrack?.addEventListener?.("ended", () =>
      console.warn("[render-video] canvas video track ended before recorder stopped"),
    );
    ...
    if (ios && !requestedMimeType.includes("mp4")) {
      throw new Error(
        "iPhone не поддържа MP4 в този браузър. Моля, превключи на 'Сървърно рендиране' за професионално 1080p качество без прекъсване.",
      );
    }
    ...
    if (opts.ayahBounds && Array.isArray(opts.ayahBounds) && opts.ayahBounds.length > 0) {
      ...
      const totalEngLen =
        bounds.reduce((acc: number, b) => acc + (b.english ? b.english.length : 10), 0) || 1;
      let wordIdx = 0;
      for (let bIdx = 0; bIdx < bounds.length; bIdx++) {
        ...
        if (b.bulgarian && typeof b.bulgarian === "string" && b.bulgarian.trim().length > 0) {
          ...
          const count = isLast
            ...
            : Math.max(1, Math.round(allWords.length * ratio));
    ...
    if (hasBgSegments) {
      ...
      for (let bIdx = 0; bIdx < bounds.length; bIdx++) {
        ...
        const interpolateSegTime = (frac: number, segs?: Array<{ start: number; end: number }>) => {
    ...
    const saveBgSnapshot = () => {
      try {
        bgSnapshot = ctx.getImageData(0, 0, W, H);
      } catch {
        /* ignore */
      }
    };
    ...
    const hasAyahBounds =
      opts.ayahBounds && Array.isArray(opts.ayahBounds) && opts.ayahBounds.length > 0;
    ...
    const drawFrame = (elapsed: number) => {
      ...
      for (let i = 0; i < phraseRender.length; i++) {
        if (hasAyahBounds) {
          ...
          if (phraseRender[i].start <= elapsed + 0.02) activePhraseIdx = i;
          else break;
        }
      }
      ...
      if (
        MIN_PHRASE_DISPLAY > 0 &&
        lastPhraseIdx >= 0 &&
        activePhraseIdx > lastPhraseIdx &&
        currentPhraseShownSince >= 0
      ) {
      ...
      const lastPhraseTimingDone =
        ...
        (activePhraseIdx === phraseRender.length - 1 &&
          elapsed >= phraseRender[phraseRender.length - 1].end - 0.01);
        ...
        if (ready) {
          try {
            ...
            drawCover(
              ctx,
              bgVideo,
              bgVideo.videoWidth || 1080,
              bgVideo.videoHeight || 1920,
              t,
              false,
            );
      ...
      if (activePhrase) {
        ...
        const themeColor =
          opts.tiktokTheme === "emerald"
            ? "#32CD32"
            : opts.tiktokTheme === "neon"
              ? "#00FFFF"
              : "#FFD700";
        ...
        for (let i = 0; i < activePhrase.lines.length; i++) {
          ...
          const totalLineWidth = lineWords.reduce((acc, w, idx) => {
            return (
              ...
              (idx < lineWords.length - 1 ? ctx.measureText(" ").width : 0)
            );
          }, 0);
    ...
    if (bgVideo) {
      try {
        await withTimeout(bgVideo.play(), 3_000, "Фоновото видео не стартира навреме");
      } catch {
        /* ignore */
      }
      await new Promise<void>((resolve) => {
        if (ios) setTimeout(resolve, 15);
    ...
    if (audioCtx && audioSource) {
      if (audioCtx.state === "suspended") {
        try {
          await withTimeout(audioCtx.resume(), 3_000, "Аудио системата не стартира навреме");
        } catch {
          /* ignore */
        }
      }
    ...
    await new Promise<void>((resolveDraw) => {
      ...
      const scheduleDraw = () => {
        if (ios) {
          rafId = window.setTimeout(draw, 15);
        } else {
      ...
      function draw() {
        ...
        const playbackDone = Boolean(
          wallDone || audioTailDone || audioFallbackDone || silentVideoDone,
        );
        ...
        if (!playbackDone) {
          ...
          console.log(
            `[render-video] FINISH: wall=${wall.toFixed(2)}s duration=${duration.toFixed(2)}s`,
          );
          finish();
        }
      }
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      ...
      recorder.onerror = (e) => {
        console.error("[render-video] recorder error", e);
        finalize();
      };
      recorder.ondataavailable = (e) => {
        if (e.data.size) chunks.push(e.data);
      ...
      const fallbackTimer = window.setTimeout(
        () => {
          if (chunks.length) finalize();
            ...
            reject(
              new Error(
                "iPhone/Safari не финализира видеото поради недостиг на памет. Моля, превключи на 'Сървърно рендиране (Фонов режим)'.",
              ),
            );
        },
        ios ? 20_000 : 5_000,
    ...
    if (blob.size < 1024) {
      throw new Error(
        "Видеото не се записа правилно в браузъра. Моля, превключи на 'Сървърно рендиране (Фонов режим)' за 100% гарантиран резултат.",
      );
    }
    if (ios && !recordedMimeType.includes("mp4")) {
      throw new Error(
        "iPhone получи неподдържан формат от браузъра. Превключи на 'Сървърно рендиране (Фонов режим)'.",
      );
    }
    return { blob, mimeType: recordedMimeType };
  } finally {
    if (audioSource && audioEndedAtWall === null) {
      try {
        audioSource.stop();
      } catch {
        /* ignore */
